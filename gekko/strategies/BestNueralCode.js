...
var strategy = {
  ...
  init: function () {
    ...
    this.predictionMinPercent = this.settings.minPredictionAccuracy || 15;
    // smooth the input to reduce the noise of the incoming data
    this.SMMA = new SMMA(3);

    let layers = [
      { type: 'input', out_sx: 1, out_sy: 1, out_depth: 1 },
      { type: 'fc', num_neurons: this.layer_neurons * 2, activation: this.layer_activation },
  ...
  check: function (candle) {
    ...
    if (this.bestLossPercent * 0.9 > this.lastPredictionLossPercent) {
    ...
    if ('sell' !== this.prevAction) {
      ...
      if (price > Math.round(this.trailPrice * 1.0025)) {
        this.trailPrice = Math.round(price * 0.9975);
      }
    }
    if ('buy' !== this.prevAction) {
      if (price < Math.round(this.trailPrice * 0.9975)) {
