...
class Ga {
  ...
  getConfig(data) {
    ...
    //   from: '2020-09-15 22:14', to: '2021-03-06 21:08'//moment(date).subtract(122, 'minutes').format('YYYY-MM-DD HH:mm')
    // };//, to: date };
  ...
  async fitnessApi(testsSeries) {
    ...
    const results = await this.queue(testsSeries, numberOfParallelQueries, async (data) => {
      ...
      let result = { profit: 0, metrics: true };
  ...
  async run() {
    ...
    while (1) {
      ...
      if (this.mainObjective == 'score') {
        ...
        if (otherPopulationMetrics[position].relativeProfit >= allTimeMaximum.otherMetrics.relativeProfit && otherPopulationMetrics[position].trades >= 2) {
          this.notifynewhigh = true;
          allTimeMaximum.parameters = population[position];
          allTimeMaximum.otherMetrics = otherPopulationMetrics[position];
          allTimeMaximum.score = score;
