...
class GaBinance {
    ...
    async loadBreakPoint() {
        ...
        if (!this.realtimeStrategy) {
            ...
            filename2 = `C:/Users/CalvinPc/gekko/gekkoga/results/realtime/${this.configName}-${this.currency}_${this.asset}_${this.realtimeStrategy.name}.json`;
        }
    ...
    runEpoch(population, populationProfits, populationSharpes, populationScores, lastActions, amountOfTrades) {
        let selectionProb = [];
        let fitnessSum = 0;
        let maxFitness = [0, 0, 0, 0, 0];
        ...
        for (let i = 0; i < this.populationAmt; i++) {
            ...
            if (this.mainObjective == 'score') {
                ...
                // console.log(this.realtimeStrategy.lastAction, 'vs', lastActions[i]);
    ...
    getConfig(data) {
        if (this.realtimeStrategy) {
            var date = new Date();
            var yesterday = date - 1000 * 60 * 60 * 24 * 7;   // current date's milliseconds - 1,000 ms * 60 s * 60 mins * 24 hrs * 1(1# of days beyond one to go back)
            yesterday = new Date(yesterday);
            // console.log(moment(date).subtract(121, 'minutes').format('YYYY-MM-DD HH:mm'));
            this.baseConfig.backtest.daterange = {
                from: '2021-02-01 07:14', to: moment(date).subtract(121, 'minutes').format('YYYY-MM-DD HH:mm')
            };//, to: date };
        }
    ...
    async fitnessApi(testsSeries) {
        ...
        const results = await this.queue(testsSeries, numberOfParallelQueries, async (data) => {
            ...
            if (report) {
                ...
                // console.log(body);
                result = { profit: body.performanceReport.profit, sharpe: body.performanceReport.sharpe, metrics: picked, lastAction: body.trades[body.trades.length - 1].action, amountOfTrades: body.trades.length, strategyParameters: body.strategyParameters };
        ...
        for (let i in results) {
            ...
            if (results.hasOwnProperty(i)) {
                ...
                lastActions.push(results[i]['lastAction']);
                amountOfTrades.push(results[i]['amountOfTrades']);
                strategyParameters.push(results[i]['strategyParameters']);
            }

        }
        return { scores, profits, sharpes, otherMetrics, lastActions, amountOfTrades, strategyParameters };
    }
    ...
    updateNewBest(allTimeMaximum) {
        ...
        if (this.realtimeStrategy) {  //if it exists we running live. 
            ...
            console.log(allTimeMaximum.otherMetrics.relativeProfit);
        } else {
            if (allTimeMaximum) {
                console.log('!!!!!!!!!Strat has been updated!!!!!!!')
                console.log(`
    ...
    async run() {
        ...
        if (loaded_config) {
            ...
            this.updateNewBest(allTimeMaximum);
        ...
        while (1) {
            ...
            if (this.mainObjective == 'score') {
                if (score > allTimeMaximum.score) {
                    ...
                    allTimeMaximum.age = new Date();
                    if (this.realtimeStrategy.lastAction !== '') {
                        allTimeMaximum.lastAction = this.realtimeStrategy.lastAction;
                    }
                    this.updateNewBest(allTimeMaximum);
                }
            } else if (this.mainObjective == 'profit') {
                ...
                // console.log(Math.round(1.10 + ((recentTime.valueOf() - allTimeMaximum.age.valueOf()) / (1000 * 60))));
                if (profit > allTimeMaximum.profit || (allTimeMaximum.age.valueOf() < recentTime.valueOf() && profit * Math.round(1.10 + ((-allTimeMaximum.age.valueOf() + recentTime.valueOf()) / (1000 * 60))) > allTimeMaximum.profit)) {
                    // console.log('score mod', highScoreMod);
                    this.notifynewhigh = true;
                    allTimeMaximum.parameters = population[position];
                    allTimeMaximum.otherMetrics = otherPopulationMetrics[position];
            ...
            if (this.realtimeStrategy)
                if ((profit * 105 / 100 >= allTimeMaximum.profit && sharpe * 105 / 100 >= this.minSharpe && new Date() - allTimeMaximum.age > 1000 * 60 * 4) || new Date() - allTimeMaximum.age > 1000 * 60 * 25 || allTimeMaximum.profit < profit) {
                    let container = {
                        parameters: {},
                        otherMetrics: {}
                    };
                    ...
                    if (fs.existsSync(`C:/Users/CalvinPc/gekko/gekkoga/results/realtime/gekkoga/nearFit/${this.configName}-${this.currency}_${this.asset}_${this.realtimeStrategy.name}.json`)) {
                        let content;
                        content = fs.readFileSync(`C:/Users/CalvinPc/gekko/gekkoga/results/realtime/gekkoga/nearFit/${this.configName}-${this.currency}_${this.asset}_${this.realtimeStrategy.name}.json`, { encoding: 'utf8' });
                        // console.log(content);

                        if (!String(content).includes(json2)) {
                            fs.appendFile(`C:/Users/CalvinPc/gekko/gekkoga/results/realtime/gekkoga/nearFit/${this.configName}-${this.currency}_${this.asset}_${this.realtimeStrategy.name}.json`, json2 + '\n', 'utf8').catch(err => console.log(err));
                        }

                    } else {
                        fs.writeFile(`C:/Users/CalvinPc/gekko/gekkoga/results/realtime/gekkoga/nearFit/${this.configName}-${this.currency}_${this.asset}_${this.realtimeStrategy.name}.json`, json2 + '\n', 'utf8').catch(err => console.log(err));
                    }
    ...
    `,
            ...
            if (!this.realtimeStrategy) {  //if it exists we running live. 
                ...
                await fs.writeFile(`C:/Users/CalvinPc/gekko/gekkoga/results/realtime/${this.configName}-${this.currency}_${this.asset}_${this.realtimeStrategy.name}.json`, json, 'utf8').catch(err => console.log(err));
            }
...
module.exports = GaBinance;
// module.exports = GaBinance;   from: moment(yesterday).format('YYYY-MM-DD HH:mm'), to: moment(date).subtract(121, 'minutes').format('YYYY-MM-DD HH:mm')