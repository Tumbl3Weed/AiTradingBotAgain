...
Trader.prototype.processAdvice = function (advice) {
  ...
  if (this.order) {
    ...
    if (direction === this.order.side) {
      this.order.editLimit(advice.price);
      console.log('Called Edit Limit 1');
      return;
    }
  ...
  if (direction === 'buy') {
    this.price = this.price * 0.9999;
    amount = (this.portfolio.currency / this.price) * 0.98;
    // if (amount < 0.0005) {
    ...
    this.price = this.price * 1.0001;
    amount = this.portfolio.asset * 0.98;
    // console.log('Asset', this.portfolio.asset, 'amount', amount);
    // if (amount < 0.0005) {
...
Trader.prototype.createOrder = function (side, amount, advice, id) {
  ...
  this.order = this.broker.createOrder(type, side, amount, params = { limit: limit, noLimit: false, outbid: true, currency: this.portfolio.currency, canTakeInstantProfit: advice.canTakeInstantProfit });
