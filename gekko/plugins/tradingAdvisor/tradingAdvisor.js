...
Actor.prototype.setupStrategy = function () {
  ...
  this.strategy.on('processRoundtrip', this.processRoundtrip)
  this.strategy.on('sendStratUpdates', this.processCandleSizeChangeRealtime)
  this.strategy
...
Actor.prototype.processCandleSizeChangeRealtime = function (candleSize) {
  this.batcher = new CandleBatcher(candleSize);
  console.log('logging from processcandlesizerealtime');
  this.batcher
    .on('candle', _candle => {
      const { id, ...candle } = _candle;
      this.deferredEmit('stratCandle', candle);
...
Actor.prototype.processRoundtrip = function (rt) {
  this.strategy.processRoundT(rt);
}
