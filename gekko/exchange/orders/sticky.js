...
class StickyOrder extends BaseOrder {
  ...
  constructor({ api, marketConfig, capabilities }) {
    ...
    // global async lock
    this.sticking = false;
    this.CanTakeInstantProfit = false;
    // bound helpers
  ...
  create(side, rawAmount, params = {}) {
    // console.log('!!!!!!!!!!!!!!!!!!!!params = ' + side + params.limit + params.outbid + params.noLimit, 'can take instant profit', params.canTakeInstantProfit);
    if (this.completed || this.completing) {
      return false;
    }
    this.CanTakeInstantProfit = params.CanTakeInstantProfit;
    this.currency = params.currency;
    console.log(new Date, 'sticky create', side, 'current currency', this.currency, 'price', params.limit, 'amount', rawAmount);

    this.side = side;

    this.amount = this.roundAmount(rawAmount);

    this.initialLimit = params.limit;
  ...
  calculatePrice(ticker) {
    ...
    if (this.side === 'buy') {
      console.log('Limit of: ' + this.limit + ' vs bid of: ' + ticker.bid);
      if (this.CanTakeInstantProfit) {
        if (ticker.ask <= this.limit) {
          this.amount = (this.currency) / ticker.ask;
          return r(ticker.ask);
        }
      } else {
        if (ticker.ask <= this.limit) {
          this.amount = (this.currency) / ticker.bid;
          return r(ticker.bid);
        }
      }
      if (!this.noLimit && ticker.bid >= this.limit) {
        // console.log('!!!!!!!!!!!!!!!!!  reached limit  !!!!!!!!!!!!!!!');
        this.amount = (this.currency) / this.limit;
        return r(this.limit);
      }
      ...
      if (outbidPrice <= this.limit && outbidPrice < ticker.ask) {
        this.amount = (this.currency) / outbidPrice;
      ...
      console.log('Limit of: ' + this.limit + ' vs ask of: ' + ticker.ask);
      if (this.CanTakeInstantProfit) {
      ...
      if (outbidPrice >= this.limit && outbidPrice > ticker.bid) {
        ...
        console.log('!!!!!!!!!!!!!!!!!  reached outbid limit  !!!!!!!!!!!!!!!');
        return r(this.limit);
      }
    }
  }
  ...
  scheduleNextCheck(numberOfCheckedTimes) {
    ...
    // register check
    if (this.orders)
      this.timeout = setTimeout(this.checkOrder, this.checkInterval * numberOfCheckedTimes);
    ...
    this.api.checkOrder(this.id, (err, result) => {
      ...
      if (result.open) {
        ...
        this.api.getTicker((err, ticker) => {
          ...
          // note: might be string VS float
          if (this.limit != this.price) {     //stop moving in place, already the correct price.
            // if (this.limit != this.calculatePrice(ticker))
  ...
  handleError(error) {
    ...
    if (error.message === 'invalid order_id') {
      delete this.orders[this.id];
      return false;
    }
  ...
  editLimit(newLim) {
    console.log('Editing Limit price to: ', newLim, 'from ', this.limit);
    // this.adjustLimit = true;
    if (this.limit === newLim)
      return;
    this.limit = newLim;
    // this.moveLimit(newLim);
    return this.move(this.limit);
